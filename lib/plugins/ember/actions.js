...
const { giveCurrency } = require('../cake/actions')
...
function getLoot(user, encounter, config) {
  ...
  if (chance <= 20 && equipment.name !== DEFAULT_WEAPON && inventorySoulless <= 10) {
    lootLog.push(
...
async function deleteItem(context, args) {
  ...
  await giveCurrency(author, amount, false)

  return context.channel.send(message(`You've sold your **${equipment.name}** for **${amount}** ${CAKE}.`))
}
