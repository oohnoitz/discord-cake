...
async function explore(context) {
  ...
  if (!encounter.name) {
    ...
    return context.channel.send(new RichEmbed().setDescription(adventureLog.join('\n')).setColor(0xff0000))
  }
...
async function deleteItem(context, args) {
  ...
  if (inventory.total === 0) {
    return context.channel.send(new RichEmbed().setDescription(`You don't have any weapons to drop...`))
  }
  ...
  if (!equipment) {
    return context.channel.send(new RichEmbed().setDescription(`You don't have that item in your inventory!`))
  }

  if (user.darkSouls.equipment === equipment.name) {
    await updateUserEquipmentItem(user, DEFAULT_WEAPON)
  }

  await removeUserInventoryItem(user, equipment.name)

  return context.channel.send(new RichEmbed().setDescription(`You've dropped your **${equipment.name}**.`))
}
...
async function updateItem(context, args) {
  ...
  if (inventory.total === 0) {
    return context.channel.send(new RichEmbed().setDescription(`You don't have any weapons to equip...`))
  }
  ...
  if (isBareHanded) {
    return context.channel.send(new RichEmbed().setDescription(`You're already using your **${DEFAULT_WEAPON}**!`))
  }
  ...
  if (useBareHands) {
    ...
    return context.channel.send(
      new RichEmbed().setDescription(`You've unequipped your **${user.darkSouls.equipment}**!`),
    )
  }
  ...
  if (user.darkSouls.equipment === equipment.name) {
    return context.channel.send(new RichEmbed().setDescription(`You already have that weapon equipped!`))
  }

  await updateUserEquipmentItem(author, equipment.name)

  return context.channel.send(new RichEmbed().setDescription(`You successfully equipped the **${equipment.name}**.`))
}
