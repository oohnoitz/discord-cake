...
async function upgradeUseInventoryItem(user, item) {
  const currency = _.get(user, 'currency.amount', 0)
  ...
  if (item.enhancement === UPGRADE_MAX) {
    return message(`Sorry, ${user}. You can't upgrade **${item.name}** any further.`).setColor(0xff0000)
  }
  ...
  if (currency >= cost) {
    ...
    return message(
      `Congratulations, ${user}. You've upgraded your equipped item to **${enhanced}** with **${cost.toLocaleString()}** ${CAKE}.`,
    ).setColor(0x008000)
  }

  return message(
    `Sorry, ${user}. It costs ${cost.toLocaleString()} ${CAKE} to upgrade your equipped item to **${enhanced}**.`,
  ).setColor(0xff0000)
}

async function removeUserInventoryItem(user, item) {
  const inventory = getInventory(user)
  const inventorySize = item.isSoulless ? inventory.size - 10 : inventory.size
...
async function upgradeItem(context) {
  ...
  const result = await upgradeUseInventoryItem(author, equipment)
