...
async function explore(context) {
  ...
  const message = new RichEmbed()
    ...
    .setFooter(`Roll: ${roll} | Chance: ${winFormula.join(' ')} = ${winChance}`)
...
async function viewRecord(context) {
  ...
  const message = new RichEmbed().setDescription(
    `${user} possesses the ${record.item}, the darkest weapon in existence materialized from ${record.score} Souls.`,
  )
