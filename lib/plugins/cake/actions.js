  ...
  return context.channel.send(
    new RichEmbed().setDescription(
      currency === 0
        ...
        : `${author}, you current have ${user.currency.amount.toLocaleString()} ${CAKE}!`,
    ),
  )
}
...
async function giveCurrency(context) {
  ...
  if (isGreedy) {
    ...
    return context.channel.send(
      new RichEmbed().setDescription(
        `Don't be greedy ${author}. You can only ask for ${CAKE} once every ${COOLDOWN} minutes. The next time you can ask for ${CAKE} is in ${countdown}.`,
      ),
    )
  }
